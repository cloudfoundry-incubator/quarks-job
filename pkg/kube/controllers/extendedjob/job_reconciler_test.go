...
var _ = Describe("ReconcileExtendedJob", func() {
	...
	Context("With a succeeded Job", func() {
		...
		It("handles an error when getting job's extended job reference failed", func() {
			job.ObjectMeta.OwnerReferences = []metav1.OwnerReference{}

			_, err := reconciler.Reconcile(request)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("could not find parent ExtendedJob reference for Job"))
		})

		It("handles an error when getting job's extended job parent reference failed", func() {
			client.GetCalls(func(context context.Context, nn types.NamespacedName, object runtime.Object) error {
				switch object := object.(type) {
				case *batchv1.Job:
					job.DeepCopyInto(object)
			...
			_, err := reconciler.Reconcile(request)
			Expect(err).To(HaveOccurred())
			Expect(err.Error()).To(ContainSubstring("getting parent ExtendedJob in Job Reconciler for job"))
		})
