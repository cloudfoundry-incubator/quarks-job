...
const (
	// MeltdownDuration is the duration of merging consecutive triggers.
...
func (r *ErrandReconciler) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	// negativeTime is used here to ensure that the time
	// is not same as now() otherwise meltdown implementation
	// will fail.
	...
	if qJob.Status.LastReconcile.Equal(&negativeTime) || qJob.Status.LastReconcile == nil {
		...
		return reconcile.Result{RequeueAfter: MeltdownDuration}, nil
	}

	if meltdown.NewWindow(MeltdownDuration, qJob.Status.LastReconcile).Contains(time.Now()) {
